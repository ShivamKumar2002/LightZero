...
class WorldModel(nn.Module):
    ...
    def compute_loss(self, batch, target_tokenizer: Tokenizer=None, inverse_scalar_transform_handle=None, **kwargs: Any) -> LossWithIntermediateLosses:
        ...
        self.visualize_reward_value_img_policy(original_images, reconstructed_images, target_predict_value, true_rewards, target_policy, predict_value, predict_rewards, predict_policy, not_plot_timesteps=[], suffix='pong_H10_H4') # TODO
    ...
    def visualize_reward_value_img_policy(self, 
        original_images, reconstructed_images, target_predict_value, true_rewards,
        ...
        for batch_idx in range(batch_size):
            ...
            ax[0].scatter(plot_timesteps, predict_rewards_filtered, color='r', label='Predict Rewards', marker='x', s=80)
            ax[0].set_xticks(plot_timesteps)
            ...
            for i, t in enumerate(plot_timesteps):
                ...
                ax[1].text(left + image_width / 2, bottom - 0.05, f'{t}', ha='center', va='top', fontsize=10)  # 添加时间步索引标签
            ...
            for i, t in enumerate(plot_timesteps):
                for j in range(num_actions):
                    ax[2].bar(i + offset[j], predict_policy[batch_idx, t-1, j].item(), width=bar_width, color=colors[j], alpha=0.5)
            ...
            for i, t in enumerate(plot_timesteps):
                for j in range(num_actions):
                    ax[3].bar(i + offset[j], target_policy[batch_idx, t-1, j].item(), width=bar_width, color=colors[j], alpha=0.5)
    ...
    def save_as_image_with_timestep(self, batch_tensor, suffix='pong'):
    ...
    def visualize_reconstruction_v3(self, original_images, reconstructed_images, target_predict_value, true_rewards, target_policy, predict_value, predict_rewards, predict_policy, not_plot_timesteps=[], suffix='pong', width=64):
        # 确保输入张量的维度匹配
        assert original_images.shape[0] == reconstructed_images.shape[0] // original_images.shape[1]
        assert original_images.shape[1] == reconstructed_images.shape[0] // original_images.shape[0]
        assert original_images.shape[2:] == reconstructed_images.shape[1:]
        ...
        for batch_idx in range(batch_size):
            ...
            for i in range(num_timesteps):
                if i + 1 not in not_plot_timesteps:
                    for j in range(num_actions):
                        ax[3].bar(plot_timesteps.index(i + 1) + offset[j], predict_policy[batch_idx, i, j].item(), width=bar_width, color=colors[j], alpha=0.5)
                        ax[4].bar(plot_timesteps.index(i + 1) + offset[j], target_policy[batch_idx, i, j].item(), width=bar_width, color=colors[j], alpha=0.5)
    ...
    def save_as_image(self, batch_tensor, suffix='pong'):
        # batch_tensor 的形状应该是 [batch_size, sequence_length, channels, height, width]
        # 在这里 channels = 4, height = 5, width = 5
        batch_size, sequence_length, channels, height, width = batch_tensor.shape
